...
public class UserHandlerServlet extends HttpServlet {
    ...
    private ArrayList<AvailableGoods> MakeAvailableGoodsForReport(Date to) throws IOException {
        ...
        if (from.before(availableList.get(0).getSnapshot_date())) {
            ArrayList<AvailableGoods> tmp = makeAvailableGoodsForReports(connection, new ArrayList<>(), new ArrayList<>(), from, availableList.get(0).getSnapshot_date());
            tmp.addAll(availableList);
            availableList = tmp;
        }
        if (to.after(availableList.get(availableList.size()-1).getSnapshot_date())) {
            ArrayList<Entity> import_available = (ArrayList<Entity>)(ArrayList<?>) availableList;
    ...
    private void rebuildSummaryAvailableGoods(Connection connection, Date from, Date last) throws IOException {
        ...
        while (current.before(last)) {
            ...
            daoAvailableGoods.AddEntityList(connection, (ArrayList<Entity>)(ArrayList<?>) makeAvailableGoodsForReports(connection, imported_available, exported_available, current_boundaries.getKey(), current_boundaries.getValue()));
